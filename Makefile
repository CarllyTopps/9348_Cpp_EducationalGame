...
ifeq ($(PLATFORM),PLATFORM_DESKTOP)
    ifeq ($(PLATFORM_OS),WINDOWS)
        ...
        # Link statically so users don't need libgcc or libstdc++ dlls in the same folder
        LDLIBS += -static -static-libgcc -static-libstdc++ -lpthread
    endif
    ifeq ($(PLATFORM_OS),LINUX)
...
# Define all source files found recursively in SRC_DIR
